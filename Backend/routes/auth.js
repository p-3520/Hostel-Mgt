...
router.post(
  ...
  async (req, res) => {
    ...
    if (!errors.isEmpty()) {
      return res
        .status(400)
        .json({ message: errors.array()[0].msg, response: false }); //agar empty nahi hain means error hain
    }
      ...
      try {
        ...
        if (!user) {
          ...
          if (!passwordcompare) {
            ...
            let profilec;
            if (user.photo_url) profilec = true;
            else profilec = false;
            ...
            if (!room_no) {
              ...
              res.json({
                jwtData,
                succes: `${user.name} This is your jwt token for the today `,
                action: "room booking is pending",
                room_booked: false,
                profile: profilec,
