...
router.post(
  "/bookroom",
  register_verify,
  [body("room", "Select a valid Room").isInt({ min: 1, max: 2000 })],
  async (req, res) => {
    ...
    if (!errors.isEmpty()) {
      ...
      return res
        .status(400)
        .json({ message: errors.array()[0].msg, reponse: false }); //agar empty nahi hain means error hain
    } else {
      ...
      if (usera.room) {
        ...
        return res
          .status(400)
          .json({ message: "User already had booked a room", response: false });
      }
      ...
      try {
        ...
        if (room) {
          return res
            .status(400)
            .json({ message: "Room is already booked", response: false });
        } else {
          ...
          let student = await User.updateOne(
            { _id: usera._id },
            { $set: { room: true } }
          );
          res.json({
            message: "Room regestration done &#128077;,",
            response: true,
          });
        }
      } catch (error) {
