...
router.get("/newcomplain", fetchuser, async (req, res) => {
  try {
    ...
    res.json({
      allcomps: allcomps,
      history_lenght: allcomps.length,
      response: true,
    });
  } catch (error) {
    console.log(error);
...
router.get("/complains", async (req, res) => {
  try {
    const allcomps = await Complain.find();
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (fuser.length && fuser[0].user == req.user) {
        let ugatet = await Complain.updateOne(
          { _id: req.body.id },
          { $set: { status: "Solved" } }
        );
      ...
      res
        .status(500)
        .json({ message: "please try again latter", response: false });
    }
  }
);
